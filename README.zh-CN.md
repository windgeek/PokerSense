...
**还没定的问题是：这个现状能不能接受**，还是说识别应该做到不用逐平台标定模板、扔进任意平台的截图
都能认。现实里能走到"不用标定"这条路，基本只有一条：**用视觉大模型（VLM）**——把这一帧图丢给一个
多模态模型，直接问它手牌/底池是什么，而不是拿像素去比对。这个方案真正能做到跨皮肤通用、不用标定，
代价是延迟（现在模板匹配大约 12ms，VLM 是几百毫秒到几秒量级）、每次调用有成本、要联网、而且输出
没有模板匹配那么确定可靠。有意思的是，架构里本来就预留了这个位置——Fast/Slow path 拆分
（`architecture.md` §4）设计的初衷就是"现在走缓存/确定性快路径，以后走 LLM/Solver 慢路径"——VLM
识别器天然应该长在 Slow Path 这个位置，不是要推翻 Fast Path 重写。

这件事现在还没有定论。这是一个真正会改变下一个里程碑工作量的范围决策（现在就按平台标定 vs. 投入做
基于 VLM 的通用识别），在正式定下来之前，先在仓库之外讨论。
